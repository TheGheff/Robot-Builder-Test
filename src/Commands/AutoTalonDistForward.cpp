...
////	changeControlMode(ControlMode.Position); //Change control mode of talon, default is PercentVbus (-1.0 to 1.0)
	...
	RobotMap::driveTrainDriveLeft->Config_kI(0, 0.0, 1000);
	RobotMap::driveTrainDriveLeft->Config_kD(0, 0.0, 1000);
...
//	RobotMap::driveTrainDriveLeft->setPosition(9158);
	int absolutePosition = RobotMap::driveTrainDriveLeft->GetSelectedSensorPosition(0) & 0xFFF; /* mask out the bottom12 bits, we don't care about the wrap arounds */


	RobotMap::driveTrainDriveLeft->Set(ControlMode::Position, 20*2048); /* 10 rotations in either direction */
}
...
void AutoTalonDistForward::End() {
	...
	RobotMap::driveTrainDriveLeft->Set(ControlMode::Velocity,0);
	RobotMap::driveTrainDriveRight->Set(ControlMode::Velocity,0);
}
